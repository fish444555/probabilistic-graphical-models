...
if (doLogZ)
    ...
    %% others
    % http://code.google.com/p/probabilistic-graphical-models/source/browse/trunk/PGM_Programming_Assignment_7/CliqueTreeCalibrate.m?r=5
    newFactor = P.cliqueList(1);
    for i=1:size(unnormalizedMessages,1)
        newFactor = FactorProduct(newFactor, unnormalizedMessages(i,1));
    end
    % newFactor = FactorProduct(P.cliqueList(1), newFactor);
    logZ = log(sum(newFactor.val));
    %% others end
