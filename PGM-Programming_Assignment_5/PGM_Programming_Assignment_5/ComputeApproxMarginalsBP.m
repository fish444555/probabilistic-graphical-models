...
function M = ComputeApproxMarginalsBP(F,E)
    ...
    C = repmat(struct('clusterList',0),1,1);
    C.clusterList = P.clusterList(length(N)+1:end);
    for i = 1:length(N),
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        indx = 0;
        for j = length(N)+1:length(P.clusterList)
            indx = FindPotentialWithVariable(C,P.clusterList(i).var);
            if(indx ~= 0)
                V = setdiff( P.clusterList(indx+length(N)).var, [i]);
                M(i) = FactorMarginalization(P.clusterList(i), V);
