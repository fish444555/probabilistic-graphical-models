...
for i = 1:length(G.names)
    ...
    % where distribution is a vector of probability distributions you get 
    % from BlockLogDistribution for variable i after you convert it from 
    % log-space and normalize it.
    
    LogBS = BlockLogDistribution(i, G, F, A);
    LogBS = exp(LogBS);
    LogBS = LogBS/sum(LogBS);
