...
%% others average time 0.0011
% idx = G.var2factors{V};
% FTemp = F(idx);
% ATemp = repmat(A,2,1);
% ATemp(:,V) = repmat([1:d],length(V),1);
% log_temp = 0.0;
% for i = 1:length(FTemp)
%     log_temp = log_temp+log(GetValueOfAssignment(FTemp(i), ATemp(:,FTemp(i).var)));
% end
% LogBS = log_temp;
%% part1 end

%% part2 average time 0.0051
% idx = G.var2factors{V};
% FTemp = F(idx);
% LogBS(2) = LogProbOfJointAssignment(FTemp, A);
% A(:,V) = 1:d;
% LogBS(1) = LogProbOfJointAssignment(FTemp, A);
%% part2 end


%% part3 both part1&part2 have error when run test 11
assign = repmat(A, d, 1);
