...
index = ones(1, prod(phenotypeFactor.card));
for i = 1:m
    for j = 1:n
        if(assignments(i,1)==1)
            index(1,i) = index(1,i)*alphaList((j-1)*3+assignments(i,j+1));
        else
            index(1,i) = index(1,i)*(1-alphaList((j-1)*3+assignments(i,j+1)));
        end
    end
