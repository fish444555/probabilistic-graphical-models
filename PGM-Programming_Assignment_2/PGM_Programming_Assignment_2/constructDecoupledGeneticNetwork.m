...
for i = 1:2*numPeople
    ...
    if(pedigree.parents(ii) == 0)
        factorList(i).var = i;
        factorList(i).card = length(alleleFreqs);
        factorList(i).val = alleleFreqs';
    else
        if(i<=numPeople)
            factorList(i).var = [i,pedigree.parents(i,1),pedigree.parents(i,1)+numPeople];
        else
            factorList(i).var = [i,pedigree.parents(i-numPeople,2),pedigree.parents(i-numPeople,2)+numPeople];
        end
        for k = 1:length(factorList(i).var)
            factorList(i).card = [factorList(i).card  3];
        end
        factorList(i).val = zeros(1,prod(factorList(i).card));
        assignments = IndexToAssignment(1:length(factorList(i).val), factorList(i).card);
        for j = 1:size(assignments,1)
            index(1,j) = ((assignments(j,1)==assignments(j,2))+...
                (assignments(j,1)==assignments(j,3)))/2;
        end
        factorList(i) = SetValueOfAssignment(factorList(i),assignments,index);
...
for j = 1:numPeople        
    temp(j,:) = [j,j+numPeople];
end

for i = 2*numPeople+1:3*numPeople
    k = 1;
    factorList(i).var = [i,temp(k,1),temp(k,2)];
    temp = temp(2:end,:);    
    factorList(i).card = [2];
    for j = 1:length(factorList(i).var)-1
        factorList(i).card = [factorList(i).card 3];
    end
    factorList(i).val = ones(1,prod(factorList(i).card));
    [allelesToGenotypes, genotypesToAlleles] = generateAlleleGenotypeMappers(numAlleles);
    k = 1;
    for j = 1:2:prod(factorList(i).card)        
        factorList(i).val(j) = alphaList(allelesToGenotypes(k))*factorList(i).val(j);
        factorList(i).val(j+1) = 1-factorList(i).val(j);
        k = k+1;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
