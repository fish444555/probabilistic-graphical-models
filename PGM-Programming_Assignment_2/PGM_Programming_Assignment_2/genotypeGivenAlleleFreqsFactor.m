...
for i = 1:length(genotypeVar)
    genotypeFactor.card = [genotypeFactor.card size(genotypesToAlleles,1)];
end

index = zeros(1,length(genotypeVar)+2);
for i = 1:size(allelesToGenotypes,1)
    for j = 1:size(allelesToGenotypes,2)
        index(allelesToGenotypes(i,j)) = index(allelesToGenotypes(i,j))+alleleFreqs(i)*alleleFreqs(j);
    end
end

genotypeFactor.val = index;
% genotypeFactor = SetValueOfAssignment(genotypeFactor,allelesToGenotypes,index);
